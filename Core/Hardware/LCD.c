...
CCMRAM void Lcd_WriteData_DMA(uint8_t *Data, uint32_t Size)
{
	...
	LCD_CS_CLR;								   // LCD片选引脚置低电平，选中LCD
	LCD_RS_SET;								   // LCD数据/命令引脚置高电平，设置为数据模式
	HAL_SPI_Transmit_DMA(LCD_SPI, Data, Size); // 启动 DMA 发送数据

	/* 这里主动等待 DMA 传输彻底完成，再返回
	 * 这样可以保证上层（如 LVGL 的 disp_flush）在返回前数据已经刷到屏上，
	 * 不会出现 lv_disp_flush_ready 提前调用导致的花屏问题。
	 */
	while (HAL_SPI_GetState(LCD_SPI) != HAL_SPI_STATE_READY)
		;
}
...
CCMRAM void LCD_Fill_LVGL(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, lv_color_t *color_p)
{
	...
	// **关键修复**：限制栈缓冲区大小，避免栈溢出（9.6KB栈）
	// 每批最多2000像素 = 4KB缓冲区，为栈留出足够空间
	#define MAX_BATCH_PIXELS 2000
	#define BATCH_BUFFER_SIZE (MAX_BATCH_PIXELS * 2)

	// 分批处理：小批次使用栈缓冲区，避免栈溢出
	uint8_t data[BATCH_BUFFER_SIZE];  // 固定4KB栈缓冲区
	uint32_t remaining = Pixel;
	
	while(remaining > 0) {
		uint32_t batch_size = (remaining > MAX_BATCH_PIXELS) ? MAX_BATCH_PIXELS : remaining;
		...
		while (HAL_SPI_GetState(LCD_SPI) != HAL_SPI_STATE_READY);
		Lcd_WriteData_DMA(data, batch_size * 2);
...
void LCD_Switch_Dir(uint8_t direction)
{
	...
	lv_disp_set_rotation(NULL, (lv_disp_rot_t)direction); // 设置lvgl屏幕旋转
